import Button from './Button';
import { ICartItem } from '../type';
import { useAppContext } from '../contexts/AppContext';
import { useCallback } from 'react';

const ShopItemCart = (props: ICartItem) => {
	const { name, price, image, quantity, onChangeQuantity, id } = props;
	const { removeItem } = useAppContext();

	const handleDecrease = useCallback(() => {
		onChangeQuantity({ itemId: id, type: 'decrease' });
	}, [id, onChangeQuantity]);

	const handleIncrease = useCallback(() => {
		onChangeQuantity({ itemId: id, type: 'increase' });
	}, [id, onChangeQuantity]);

	const handleRemove = useCallback(() => {
		removeItem(id);
	}, [id, removeItem]);
	return (
		<div className="cardItem">
			<div className="cardItem_left">
				<div className="cardItem_image">
					<img alt={`${name}`} src={image} loading="lazy" />
				</div>
			</div>
			<div className="cardItem_right">
				<div className="cardItem_name">{name}</div>
				<div className="cardItem_price">${price}</div>
				<div className="cartItem_actions">
					<div className="cartItem_count">
						<button className="cartItem_button" onClick={handleDecrease}>
							-
						</button>
						<div className="cartItem_number">{quantity}</div>
						<button className="cartItem_button" onClick={handleIncrease}>
							+
						</button>
					</div>
					<Button
						onClick={handleRemove}
						buttonText="REMOVE"
						className="carItem_remove"
						icon={
							<img
								alt=""
								src="data:image/png;base64,"
							/>
						}
					/>
				</div>
			</div>
		</div>
	);
};

export default ShopItemCart;
